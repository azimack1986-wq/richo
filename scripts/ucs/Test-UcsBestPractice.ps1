...
.NOTES
    REQUIREMENTS
      - Cisco UCS PowerTool (Cisco.UCSManager). Read-only rights are enough; every call is a read.
...
try {
    ...
    if (-not (Get-Command -Name 'Connect-Ucs' -ErrorAction SilentlyContinue)) {
        # Auto-loading did not find it, so name the module in the failure rather than letting the
        # run die later on "term not recognized".
